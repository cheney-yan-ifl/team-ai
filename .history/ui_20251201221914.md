...
### Layout Discussion Bullets
- Top bar stays Workspace-only for now; agree when to reintroduce search/settings.
- Left nav order: Agents first, then Topics/Channels; “New Topic” button lives here—confirm this default.
- Thread toggle meaning: simple observer/passive visibility switch—decide default state (on/off).
- Right panel focus: state snapshot + agent lineup with activity indicators + files/tasks + timeline; hooks/config deferred—align on this scope.
- Composer matches chat styling; no extra status line until a requirement emerges.
